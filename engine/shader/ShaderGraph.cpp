...
ShaderNodeID ShaderGraph::AddNode(std::unique_ptr<ShaderNode> node) {
    ShaderNodeID id = m_nextID++;
    m_nodes[id] = std::move(node);
    m_compiled = false;
    return id;
}

void ShaderGraph::RemoveNode(ShaderNodeID id) {
    m_nodes.erase(id);
    m_edges.erase(
        std::remove_if(m_edges.begin(), m_edges.end(),
            [id](const ShaderEdge& e) {
...
void ShaderGraph::AddEdge(const ShaderEdge& edge) {
    m_edges.push_back(edge);
    m_compiled = false;
}

void ShaderGraph::RemoveEdge(const ShaderEdge& edge) {
    m_edges.erase(
        std::remove_if(m_edges.begin(), m_edges.end(),
            [&edge](const ShaderEdge& e) {
...
bool ShaderGraph::HasCycle() const {
    std::unordered_map<ShaderNodeID, int> inDegree;
...
bool ShaderGraph::ValidateEdgeTypes() const {
    for (auto& e : m_edges) {
...
bool ShaderGraph::Execute(const ShaderContext& ctx) {
    if (!m_compiled) return false;

    m_outputs.clear();

    for (ShaderNodeID id : m_executionOrder) {
        ...
        std::vector<ShaderValue> inputs(inputDefs.size());
        for (auto& e : m_edges) {
        ...
        std::vector<ShaderValue> outputs(outputDefs.size());
        node->Evaluate(ctx, inputs, outputs);

        for (ShaderPortID p = 0; p < outputs.size(); ++p) {
...
const ShaderValue* ShaderGraph::GetOutput(ShaderNodeID node, ShaderPortID port) const {
...
size_t ShaderGraph::NodeCount() const {
    return m_nodes.size();
}

bool ShaderGraph::IsCompiled() const {
    return m_compiled;
}
