...
RenderNodeID RenderGraph::AddNode(std::unique_ptr<RenderNode> node) {
    RenderNodeID id = m_nextID++;
    m_nodes[id] = std::move(node);
    m_compiled = false;
    return id;
}

void RenderGraph::RemoveNode(RenderNodeID id) {
    m_nodes.erase(id);
    m_edges.erase(
        std::remove_if(m_edges.begin(), m_edges.end(),
            [id](const RenderEdge& e) {
...
void RenderGraph::AddEdge(const RenderEdge& edge) {
    m_edges.push_back(edge);
    m_compiled = false;
}

void RenderGraph::RemoveEdge(const RenderEdge& edge) {
    m_edges.erase(
        std::remove_if(m_edges.begin(), m_edges.end(),
            [&edge](const RenderEdge& e) {
...
bool RenderGraph::HasCycle() const {
    std::unordered_map<RenderNodeID, int> inDegree;
...
bool RenderGraph::ValidateEdgeTypes() const {
    for (auto& e : m_edges) {
...
bool RenderGraph::Execute(const RenderContext& ctx) {
    if (!m_compiled) return false;

    m_outputs.clear();

    for (RenderNodeID id : m_executionOrder) {
        auto it = m_nodes.find(id);
        if (it == m_nodes.end()) return false;

        RenderNode* node = it->second.get();
        auto inputDefs = node->Inputs();

        std::vector<RenderValue> inputs(inputDefs.size());
        for (auto& e : m_edges) {
        ...
        std::vector<RenderValue> outputs(outputDefs.size());
        node->Evaluate(ctx, inputs, outputs);

        for (RenderPortID p = 0; p < outputs.size(); ++p) {
...
const RenderValue* RenderGraph::GetOutput(RenderNodeID node, RenderPortID port) const {
...
size_t RenderGraph::NodeCount() const {
    return m_nodes.size();
}
